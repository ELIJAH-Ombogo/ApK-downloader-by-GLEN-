...
app.get('/api/download', async (req, res) => {
    try {
        ...
        const apiUrl = type === 'audio' 
            ? `https://kaiz-apis.gleeze.com/api/download/ytmp3?url=${encodeURIComponent(url)}`
            : `https://kaiz-apis.gleeze.com/api/download/ytmp4?url=${encodeURIComponent(url)}`;
...
app.get('/api/preview', async (req, res) => {
    try {
        ...
        const apiUrl = `https://kaiz-apis.gleeze.com/api/download/ytmp3?url=${encodeURIComponent(url)}`;
        const apiResponse = await axios.get(apiUrl);
